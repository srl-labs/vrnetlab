...
class AlmaLinux_vm(vrnetlab.VM):
    def __init__(
        self,
        hostname,
        username,
        ...
        super(AlmaLinux_vm, self).__init__(
    ...
    def gen_mgmt(self):
        ...
        res = super(AlmaLinux_vm, self).gen_mgmt()

        # we need to place mgmt interface on the same bus with other interfaces in AlmaLinux,
...
class AlmaLinux(vrnetlab.VR):
    def __init__(self, hostname, username, password, nics, conn_mode):
        super(AlmaLinux, self).__init__(username, password)
        self.vms = [AlmaLinux_vm(hostname, username, password, nics, conn_mode)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hostname", default="almalinux", help="VM Hostname")
